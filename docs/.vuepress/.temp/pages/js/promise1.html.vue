...
<span class="token comment">// 一个 Promise 的当前状态必须为以下三种状态中的一种：等待态（Pending）、执行态（Fulfilled）</span>
<span class="token comment">// 和拒绝态（Rejected）。</span>
...
<span class="token comment">//      2.1.1.1 可以迁移至执行态或拒绝态</span>
  ...
  <span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token keyword">this</span><span class="token punctuation">.</span>status <span class="token operator">===</span> <span class="token constant">PENDING</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 2.2.4 onFulfilled or onRejected must not be called until the execution </span>
    <span class="token comment">//       context stack contains only platform code.[3.1].</span>
    <span class="token comment">// execution context： https://es5.github.io/#x10.3</span>

    <span class="token comment">// 2.2.4 onFulfilled 和 onRejected 只有在执行环境堆栈仅包含平台代码时才可被调用（这里</span>
    <span class="token comment">//       的平台代码指的是引擎、环境以及 promise 的实施代码。实践中要确保 onFulfilled</span>
    <span class="token comment">//       和 onRejected 方法异步执行，且应该在 then 方法被调用的那一轮事件循环之后的新执</span>
    <span class="token comment">//       行栈中执行。这个事件队列可以采用“宏任务macro-task”机制或者“微任务micro-task”</span>
    <span class="token comment">//       机制来实现）。</span>
...
<span class="token comment">// A promise must provide a then method to access its current or eventual value</span>
<span class="token comment">//  or reason.</span>
...
<span class="token comment">// 一个 promise 必须提供一个 then 方法以访问其当前值、终值和拒因(也就是拒绝原因，指在</span>
<span class="token comment">//  promise 被拒绝时传递给拒绝回调的值）</span>
...
<span class="token class-name">PromiseA</span><span class="token punctuation">.</span>prototype<span class="token punctuation">.</span><span class="token function-variable function">then</span> <span class="token operator">=</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">onFulfilled<span class="token punctuation">,</span> onRejected</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">var</span> onFulfilled <span class="token operator">=</span> <span class="token keyword">typeof</span> onFulfilled <span class="token operator">===</span> <span class="token string">'function'</span>
  ...
  <span class="token keyword">var</span> onRejected <span class="token operator">=</span> <span class="token keyword">typeof</span> onRejected <span class="token operator">===</span> <span class="token string">'function'</span>
  ...
  <span class="token keyword">var</span> promise2 <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">PromiseA</span><span class="token punctuation">(</span><span class="token punctuation">(</span><span class="token parameter">resolve<span class="token punctuation">,</span> reject</span><span class="token punctuation">)</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">function</span> <span class="token function">foo1</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
      <span class="token keyword">try</span> <span class="token punctuation">{</span>
        ...
        <span class="token comment">//      2.2.2.1 it must be called after promise is fulfilled, with</span>
        <span class="token comment">//              promise’s value as its first argument.</span>
        ...
        <span class="token comment">// 2.2.7.1 If either onFulfilled or onRejected returns a value x, run</span>
        <span class="token comment">//         the Promise Resolution Procedure [[Resolve]](promise2, x).</span>

        <span class="token comment">// 2.2.7.1 如果 onFulfilled 或者 onRejected 返回一个值 x ，则运行下面的</span>
        <span class="token comment">//         Promise 解决过程：[[Resolve]](promise2, x)</span>
      ...
      <span class="token punctuation">}</span> <span class="token keyword">catch</span> <span class="token punctuation">(</span>e<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token comment">// 2.2.7.2 If either onFulfilled or onRejected throws an exception e, </span>
        <span class="token comment">//         promise2 must be rejected with e as the reason.</span>

        <span class="token comment">// 如果 onFulfilled 或者 onRejected 抛出一个异常 e ，则 promise2 必须拒绝执行，</span>
        <span class="token comment">// 并返回拒因 e</span>
    ...
    <span class="token keyword">function</span> <span class="token function">foo2</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
      <span class="token keyword">try</span> <span class="token punctuation">{</span>
        ...
        <span class="token comment">//      2.2.3.1 it must be called after promise is rejected, with </span>
        <span class="token comment">//              promise’s reason as its first argument.</span>

        <span class="token comment">// 2.2.3 如果 onRejected 是函数：</span>
        <span class="token comment">//      2.2.3.1 当 promise 被拒绝执行后其必须被调用，其第一个参数为promise的拒因。</span>
        ...
        <span class="token comment">// 2.2.7.1 If either onFulfilled or onRejected returns a value x, </span>
        <span class="token comment">//         run the Promise Resolution Procedure [[Resolve]](promise2, x).</span>
    ...
    <span class="token keyword">if</span> <span class="token punctuation">(</span>that<span class="token punctuation">.</span>status <span class="token operator">===</span> <span class="token constant">FULFILLED</span><span class="token punctuation">)</span> <span class="token punctuation">{</span> 
      ...
      <span class="token comment">// 2.2.4 onFulfilled 和 onRejected 只有在执行环境堆栈仅包含平台代码时才可被调用</span>
      <span class="token comment">//      （这里的平台代码指的是引擎、环境以及 promise 的实施代码。实践中要确保 </span>
      <span class="token comment">//       onFulfilled 和 onRejected 方法异步执行，且应该在 then 方法被调用的那一轮</span>
      <span class="token comment">//       事件循环之后的新执行栈中执行。这个事件队列可以采用“宏任务（macro-task）”机制</span>
      <span class="token comment">//       或者“微任务（micro-task）”机制来实现）。</span>
      <span class="token comment">// 执行上下文 ：https://es5.github.io/#x10.3</span>
    ...
    <span class="token punctuation">}</span> <span class="token keyword">else</span> <span class="token keyword">if</span> <span class="token punctuation">(</span>that<span class="token punctuation">.</span>status <span class="token operator">===</span> <span class="token constant">PENDING</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
      ...
      <span class="token comment">//      2.2.6.1 If/when promise is fulfilled, all respective onFulfilled </span>
      <span class="token comment">//              callbacks must execute in the order of their originating</span>
      <span class="token comment">//              calls to then.</span>
      ...
      <span class="token comment">//      2.2.6.2 If/when promise is rejected, all respective onRejected</span>
      <span class="token comment">//              callbacks must execute in the order of their originating</span>
      <span class="token comment">//              calls to then.</span>
...
<span class="token keyword">function</span> <span class="token function">resolvePromise</span><span class="token punctuation">(</span><span class="token parameter">promise2<span class="token punctuation">,</span> x<span class="token punctuation">,</span> resolve<span class="token punctuation">,</span> reject</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token comment">// 2.3.1 If promise and x refer to the same object, reject promise with</span>
  <span class="token comment">//       a TypeError as the reason.</span>
  ...
  <span class="token punctuation">}</span> <span class="token keyword">else</span> <span class="token keyword">if</span> <span class="token punctuation">(</span>x <span class="token keyword">instanceof</span> <span class="token class-name">PromiseA</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 2.3.2.1 If x is pending, promise must remain pending until x is </span>
    <span class="token comment">//         fulfilled or rejected</span>
 ...
 <span class="token punctuation">}</span> <span class="token keyword">else</span> <span class="token keyword">if</span> <span class="token punctuation">(</span>x <span class="token operator">!==</span> <span class="token keyword">null</span> <span class="token operator">&amp;&amp;</span> <span class="token punctuation">(</span><span class="token keyword">typeof</span> x <span class="token operator">===</span> <span class="token string">'object'</span> <span class="token operator">||</span> <span class="token keyword">typeof</span> x <span class="token operator">===</span> <span class="token string">'function'</span><span class="token punctuation">)</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    ...
    <span class="token comment">// 2.3.3.2 If retrieving the property x.then results in a thrown </span>
    <span class="token comment">//         exception e, reject promise with e as the reason.</span>
    ...
    <span class="token keyword">try</span> <span class="token punctuation">{</span>
      ...
      <span class="token keyword">if</span><span class="token punctuation">(</span><span class="token keyword">typeof</span> then <span class="token operator">===</span> <span class="token string">'function'</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token keyword">try</span> <span class="token punctuation">{</span>
          <span class="token comment">// 2.3.3.3 If then is a function, call it with x as this, </span>
          <span class="token comment">//         first argument resolvePromise, and second argument</span>
          <span class="token comment">//         rejectPromise, where:</span>
          
          <span class="token comment">// 2.3.3.3  如果 then 是函数，将 x 作为函数的作用域 this 调用之。传递</span>
          <span class="token comment">//          两个回调函数作为参数，第一个参数叫做 resolvePromise ，第</span>
          <span class="token comment">//          二个参数叫做 rejectPromise:</span>
          <span class="token function">then</span><span class="token punctuation">.</span><span class="token function">call</span><span class="token punctuation">(</span>x<span class="token punctuation">,</span> <span class="token keyword">function</span><span class="token punctuation">(</span><span class="token parameter">y</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
            ...
            <span class="token comment">// 2.3.3.3.3 If both resolvePromise and rejectPromise are called, </span>
            <span class="token comment">//           or multiple calls to the same argument are made, the </span>
            <span class="token comment">//           first call takes precedence, and any further calls</span>
            <span class="token comment">//           are ignored</span>

            <span class="token comment">// 2.3.3.3.3 如果 resolvePromise 和 rejectPromise 均被调用，或者被同一</span>
            <span class="token comment">//           参数调用了多次，则优先采用首次调用并忽略剩下的调用</span>
